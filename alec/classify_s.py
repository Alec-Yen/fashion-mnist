...
"""
def mpp(x, mu, cov, prior, case):
    c = cov.shape[0]  # how many classes (categories)
    g = np.zeros(c)

    for i in range(c):
        if case == 1:  # Case 1, using average of variance
            cov_tmp = np.mean(cov,axis=0)
            g[i] = -1/(2*cov_tmp[0][0]) * (x-mu[i]).T.dot(x-mu[i]) + np.log(prior[i])
        if case == 2:  # Case 2, using average of covariance matrices
            cov_tmp = np.mean(cov,axis=0)
            g[i] = -1/2*(x-mu[i]).T.dot(np.linalg.inv(cov_tmp)).dot(x-mu[i]) + np.log(prior[i])
        if case == 3:
            g[i] = -1/2*np.log(np.linalg.norm(cov[i])) - 1/2*(x-mu[i]).T.dot(np.linalg.inv(cov[i])).dot(x-mu[i]) + np.log(prior[i])
...
"""
def discriminant_accuracy(tr, te, prior, case):
    TP = 0
    TN = 0
    FP = 0
    FN = 0
    correct = 0
    mu,cov = pp.get_params_arr(pp.return_2class_as_array(tr)) # get parameters from training data
    for test_sample in te: # iterate through test data
        x_test = test_sample[0:-1].reshape(-1, 1)  # reshape test features to column vector
        if mpp(x_test, mu, cov, prior, case) == test_sample[-1]:
            correct += 1
            if test_sample[-1]==0:
                TN += 1
            else:
